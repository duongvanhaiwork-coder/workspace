...
3. **MCP code intelligence** — if AI Core MCP tools are in session, use them for code discovery before grep; if not, editor fallback only (`rules/cursor/mcp-code-intelligence.mdc`). Opt-out: `mcp:off`, `no-mcp`.
