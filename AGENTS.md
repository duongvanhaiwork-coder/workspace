...
3. **Code quality** — `rules/cursor/general.mdc` and stack `*.mdc` by file type always apply.
...
- If `<target-repo>` has no `docs/` or policy forbids it: ask once, then use an existing doc root (`specs/`, `design/`, `notes/`) with the same `work/YYYY-MM-DD-<slug>/` shape (see `skills/question-scope/SKILL.md`).
