...
| `explain-code` | Explain code, no edits | L1; before editing unfamiliar code | Rule **`mcp-intelligence`** → entry flow → dependencies; label graph vs search |
...
| `analyze-impact` | Which files/services are affected | L4 Discover; L3/L2 optional | Rule **`mcp-intelligence`** → impact list; **does not** replace Regression tests |
