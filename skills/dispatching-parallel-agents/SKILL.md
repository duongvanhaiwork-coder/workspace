...
Every dispatched agent **must** return a structured report. Controller parses these to integrate results.
...
- **Files changed:** [list]
- **Conflicts:** [files shared with other domains, or "none"]
- **Verify:** [command run + result, e.g. "npm test -- agent-tool-abort: 3/3 pass"]
```
...
| -------------------- | --------------------------------------------------------------- |
| `DONE`               | Read fix, check conflicts, proceed to integrate                 |
| `DONE_WITH_CONCERNS` | Read concerns before integrating — may need manual review       |
| `BLOCKED`            | Re-dispatch with more context, split task, or escalate to human |

**Include in your dispatch prompt:**
...
- **Files changed:** list
- **Conflicts:** any files that other parallel agents might also touch, or "none"
- **Verify:** test command + result
```

**Why this matters:** Without structured output, controller wastes tokens re-reading agent work to understand what happened. Structured reports enable fast conflict detection across parallel agents.
