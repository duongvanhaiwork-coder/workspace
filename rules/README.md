...
**Generated output:** `kiro/rules-*.md` is produced from Cursor rules by `scripts/sync-ide-rules.sh` (also invoked from `scripts/sync-ide.sh` / `make sync-ide`). **Do not edit `kiro/` by hand** — changes are overwritten on the next sync.
...
| From | To |
| ---- | -- |
| `rules/cursor/*.mdc` | `rules/kiro/rules-<stem>.md` (generated in repo) |
...
```text
  rules/cursor/dotnet.mdc → rules/kiro/rules-dotnet.md
  ...
...
| ---- | ------------- | ------- | -------- |
| `general.mdc` | `true` | — | Shared defaults (all files) |
| `question-scope.mdc` | `true` | — | Question Scope triggers → `skills/question-scope/` |
...
| `workflow.mdc` | `false` | — | Workflow rule IDs & handoffs (on demand; Kiro: `#rules-workflow`) |
...
rules/
  ...
  kiro/rules-*.md       ← generated by sync-ide-rules.sh
  README.md
  CONVENTIONS.md
  QUICKSTART.md
...
| `kiro/` | Kiro (`~/.kiro/steering/`) | `rules-*.md` — **generated** |
