...
Registry & playbooks: `skills/SKILLS-REGISTRY.md`, `skills/CONVENTIONS.md`. **Rules below use rule IDs**; detailed steps live in the matching **skill ID** (invoke-skill).

**With question-scope:** L1–L4 gates and `docs/work/…` come from **`question-scope`**; supplement map: **`question-scope`** → `references/superpowers-supplement.md`. This file does not replace scope level choice. When scope is active, **level + supplement table override** the feature flow and hard gates below (e.g. L2 skips design/plan; L3 default execute is **B**).
...
| `tdd-during-implementation` | `test-driven-development` |
| `verify-before-done` | `verification-before-completion` |
| `finish-branch-options` | `finishing-a-development-branch` |
| `debug-root-cause-first` | `systematic-debugging` |
| `tdd-failing-repro` | `test-driven-development` |
| `verify-fix-evidence` | `verification-before-completion` |
| `parallel-failure-domains` | `dispatching-parallel-agents` |
| `outgoing-code-review` | `requesting-code-review` |
| `incoming-code-review` | `receiving-code-review` |
...
**Playbook** = optional skill ID detail for a rule (see index; invoke-skill). Do not use legacy `superpowers:<id>` or `skills/<id>/SKILL.md` paths in rules.
...
**Question-scope active:** STOP gates and level budgets in **`question-scope`** win over this section.
