...
- **Do not** reformat the entire file — only format code within the lines being changed; leave surrounding code as-is.
- **⚠️ CRITICAL: Do not** reformat or re-style existing code under any circumstances. Agent is **ONLY allowed to change logic/code**, **ABSOLUTELY MUST NOT reformat** (indentation, spacing, line breaks, trailing commas, quote style, bracket placement, import ordering, etc.) any code not directly related to the logic change. Violating this rule is a critical error.
...
## Shell commands (agent execution)

- When a script exceeds **3 lines**: write it to a temporary file (`.py`, `.sh`) then execute that file. **Do not** use inline heredocs, `python3 -c` with multi-line code, or `bash -c` with complex logic.
- Reason: easier to debug, avoids quoting/escaping errors, and the user can inspect the file before execution.
- Clean up temporary script files after successful execution.
- For simple one-liners (single grep, single mv, etc.), inline execution is fine.
