...
- **⚠️ CRITICAL: Do not** reformat or re-style existing code under any circumstances. Agent **CHỈ ĐƯỢC PHÉP sửa logic/code**, **TUYỆT ĐỐI KHÔNG ĐƯỢC format lại code** (indentation, spacing, line breaks, trailing commas, quote style, bracket placement, import ordering, etc.) của bất kỳ phần code nào không trực tiếp liên quan đến thay đổi logic. Vi phạm rule này coi như lỗi nghiêm trọng.
